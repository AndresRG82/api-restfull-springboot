package cl.kibernumacademy.rest.security.controller;
...
import cl.kibernumacademy.rest.security.dto.AuthResponse;
// DTO para login (username/password) con validación.
import cl.kibernumacademy.rest.security.dto.LoginRequest;
// DTO para registro (username/password) con validación.
import cl.kibernumacademy.rest.security.dto.RegisterRequest;
// Propiedades de JWT (secret, issuer, TTLs, toggles, CORS...).
import cl.kibernumacademy.rest.security.jwt.JwtProperties;
// Servicio para emitir y validar tokens JWT.
import cl.kibernumacademy.rest.security.jwt.JwtService;
 ...
 * - POST /login: autentica un usuario (username/password) y devuelve accessToken (+ refreshToken si está habilitado).
 * - POST /register: registra un nuevo usuario con ROLE_USER si la característica está habilitada en propiedades.
 * - POST /refresh: recibe un refresh token (en el cuerpo como texto plano) y devuelve un nuevo accessToken (+ refreshToken).
 *
...
public class AuthController {
     ...
     // Gestor de autenticación configurado por Spring (usa nuestro UserDetailsService + PasswordEncoder).
    ...
    public ResponseEntity<AuthResponse> login(@Valid @RequestBody LoginRequest request) {
        try {
                // Delegamos la verificación de credenciales al AuthenticationManager (PasswordEncoder incluido).
            ...
            return ResponseEntity.ok(new AuthResponse(access, expiresIn, refresh));
                ...
                // Propagamos para que nuestro handler de seguridad formatee la respuesta 401 en JSON.
            throw ex;
        }
    }
      ...
      * Lanza IllegalArgumentException si el username ya existe (manejado por advice global).
      */
    ...
    public ResponseEntity<Void> register(@Valid @RequestBody RegisterRequest request) {
      ...
      * Importante: este endpoint espera el refresh token en el cuerpo como texto plano (no JSON).
      * Si refresh está deshabilitado via propiedades, responde 404 Not Found.
      * Valida la versión del token (tokenVersion) para soportar invalidación global por usuario.
      */
    @PostMapping("/refresh")
